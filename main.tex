...
Another thing to notice is that Records are not named, as structs and classes are in C++. This allows for flexible structural typing, in which the only criteria for a type to be accepted as the argument to a function is if it has the minimum set of fields. If stronger type safety is desired, extra metadata can be added to the type description to indicate type names and/or subtype relationships.

In the same way, names can distinguish a raw ``List(char)'' from a ``String'' or even a ``UTF8String,'' which should be interpreted differently by user code. Similarly, nullable data can be represented as an ``Option(X),'' which is a ``List(X)'' that only allows zero (empty, null) or one (filled) elements.

A particularly useful type is ``Pointer(X),'' which is an integer interpreted as a List index. In addition to having the ``Pointer'' name that distinguishes it from ordinary integers, this type would have metadata identifying another List in the structure; dereferencing the pointer returns the item at the List index. With this mechanism, not only can tree data be constructed, but graphs with cycles as well. It also provides a way to compactly represent subsets of datasets, since a physicist's ``event list'' is a ``List(Pointer(Events)).''

\subsection{Objects to Arrays}

Any data described by the PLUR type system can be converted into a set of named arrays. The array names all begin with a user-provided prefix, so that the user can manage arrays representing different objects in a single object store, zip file, HDF5 file, or Python dictionary. Names are generated by a recursive walk over the type structure, and in fact the type schema is losslessly preserved in the collection of names. This may be useful if an external metadata system is not available to store the type schema.
...
{\bf If $T$ is a Union} with possible types $T_1, \ldots, T_n$ and the datum has actual type $T_i$ (where $i \in [1, n]$), find or create an array named $N + \mbox{``-Ut''}$ (union tag) and append $i$.

Next, follow the rule for name $N + \mbox{``-Ud''} + i$ (union data $i$) and type $T_i$.

{\bf If $T$ is a Record} with field names and field types $(N_1, T_1), \ldots (N_n, T_n)$, follow the rule for each pair, using $N + \mbox{``-R\_''} + N_i$ (record field $N_i$) as a name and $T_i$ as a type.

Be sure to include empty/trivial arrays for types that were not touched due to missing data (Lists that are all empty at a given level or Union type possibilities that never occur in the data) or make the reading procedure insensitive to missing arrays. A simple way to do it is to create all arrays with a pass over the type schema and only fill them in the pass over data.
...
First, select arrays whose names begin with the prefix and pop the prefix from their names. For each of these arrays $a$, create an index $e_a$ whose initial value is 0. Then recursively apply the following rules.

{\bf If only one array exists and its name is the empty string}, then the type is a Primitive. Take the value at index $e_a$ and increment $e_a$ by 1.

{\bf If one array name begins with ``-Lo'' and all others begin with ``-Ld''}, then the type is a List. Take $a[e_a + 1] - a[e_a]$ as the length $\ell$ of the List and increment $e_a$ by 1 for the array $a$ that begins with ``-Lo''.

Pop the ``-Ld'' from the beginning of all other arrays and apply the rule for that set $\ell$ times.

{\bf If one array name begins with ``-Ut'', and all others begin with ``-Ud''}, then the type is a Union.

Take $a[e_a]$ as the tag $i$ for the datum and increment $e_a$ by 1 for the array $a$ that begins with ``-Ut''.

Pop the $\mbox{``-Ud''} + i$ from the beginning of all data array names associated with tag $i$ and apply the rule for that subset.

{\bf If all array names begin with ``-R\_''}, then the type is a Record. Partition the set of arrays by field name $N_i$, pop the $\mbox{``-R\_''} + N_i$ from the beginning of the array names, and apply the rule separately for each partition to fill each field.
...
The reason that List structures are represented by offsets (arrays whose names end in ``-Lo''), rather than lengths, is to permit random access.
