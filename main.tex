...
Random access is a useful feature, but it is less optimal for compression or variable-length integer encoding algorithms. If we wish to store or transmit a List structure in the fewest possible bytes, it should be converted into a corresponding size array (suffix ``-Ls''). For all $i \in [0, \mbox{ length of x-Lo} - 1)$,
...
\vspace{0.15 cm}
\FOR{$i \in [0, \mbox{ length of x-Ut}]$}
...
To reconstruct one Union object at index $i$, we find the tag $t$ at x-Ut{\tt [}$i${\tt ]} and follow the arrays-to-objects algorithm with the set of x-Ud$t$ arrays, all starting at index $i_t = \mbox{x-Uo{\tt [}}i\mbox{\tt]}$.
