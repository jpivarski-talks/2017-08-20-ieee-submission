...
{\bf If $T$ is a List} with contained type $T'$ and length $\ell$, find an array named $N + \mbox{``-Lo''}$ (list offset). If it does not yet exist, create it with a single element $0$. Then select the last element $x$ from this array and append $\ell + x$ to the end of the array.

Next, iterate through each element in the List and apply the rule for $T'$ with name $N + \mbox{``-Ld''}$ (list data).

{\bf If $T$ is a Union} with possible types $T_1, \ldots, T_n$ and actual type $T_i$ (where $i \in [1, n]$), find or create an array named $N + \mbox{``-Ut''}$ (union tag) and append $i$. Next, follow the rule for name $N + \mbox{``-Ud''} + i$ (union data $i$), type $T_i$, noting the length of the data array before insertion. Put this length in a (possibly new) array named $N + \mbox{``-Uo''}$ (union offset).

{\bf If $T$ is a Record} with field names and field types $(N_1, T_1), \ldots (N_n, T_n)$, follow the rule for each $(N_i, T_i)$ pair. Records do not generate any arrays other than those associated with each field.
