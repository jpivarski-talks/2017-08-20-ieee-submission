...
\author{\IEEEauthorblockN{Jim Pivarski}
\IEEEauthorblockA{Physics Department\\
Princeton University\\
Princeton, NJ, 08544\\
pivarski@princeton.edu}
\and
\IEEEauthorblockN{Peter Elmer}
\IEEEauthorblockA{Physics Department\\
Princeton University\\
Princeton, NJ, 08544\\
peter.elmer@cern.ch}
...
The second action a HEP analyst usually takes is to compute masses from {\tt\small pt}, {\tt\small eta}, and {\tt\small phi} of pairs of particles\footnote{$\sqrt{2 {p_1}^{p_T} {p_2}^{p_T} (\cosh({p_1}^\eta - {p_2}^\eta) - \cos({p_1}^\phi - {p_2}^\phi))}$ for $p_1$ and $p_2$}, to see if any were the decay products of a short-lived intermediary. Candidates for such a decay would always be found in the same event, so this query in SQL would involve a full outer join on two exploded {\tt\small muons} tables or exploded {\tt\small muons} and exploded {\tt\small jets}, subject to equal {\tt\small eventId}. In this case, it is even more important that the uniqueness of {\tt\small eventId} is tracked, since the join could turn an $\mathcal{O}(n_{\mbox{\scriptsize events}})$ calculation into $\mathcal{O}(n_{\mbox{\scriptsize events}} \log n_{\mbox{\scriptsize events}})$.

HEP data manipulations only get more complex from there. The problem is that the data's schema is a poor fit to the relational model for which SQL was designed. The data consist of hierarchically nested elements, which are easily expressed in a procedural or functional language as arbitrary-length lists and record objects, with many language tools designed to manipulate them.
...
ROOT\cite{root}, the HEP analysis framework, was first to store hierarchical data structures as columns. The ROOT file format (in ``split mode'') stores primitive attribute data adjacent to one another on disk, e.g.\ all muon {\tt\small pt} values, followed by all muon {\tt\small eta} values, etc. Google independently developed Dremel\cite{dremel}, on which Apache Parquet is based. Spark 2.0 and Apache Arrow extend this concept to data in memory, and each has seen substantial increases in data throughput when switching to a columnar layout, especially for processes that access only a few of the attributes.

JIT compilation is central to machine learning tools like H2O\cite{h2o} and Theano\cite{theano}, Julia\cite{julia}, a scientific programming language, as well as ROOT. To our knowledge, however, none manipulate the user's code to match the structure of its columnar data.
...
Another is that Records are not named, as structs and classes are in C++. This allows for flexible structural typing, in which an object is accepted as a function argument if it has a required set of fields, but it's weaker than nominal typing that would distinguish a ``Point3D'' with the same field names and types as a ``Vector3D.'' If the latter is desired, extra metadata can be added to the type description to indicate a name.

In the same way, names can distinguish a raw ``List(char)'' from a ``String'' or even a ``UTF8String,'' which can only be used in certain functions. Similarly, nullable data can be represented as an ``Option(X),'' which is a ``List(X)'' that only allows zero (empty, null) or one (filled) elements.

A particularly useful runtime interpretation is ``Pointer(X),'' which is an integer type with metadata pointing to another List in the structure. Integer values of the pointer are interpreted as indexes of the list: dereferencing the pointer is just selecting an item from the list. With this mechanism, not only can tree data be constructed, but graphs with cycles as well.

\subsection{Objects to Arrays}

Any data described by the PLUR type system can be converted into a set of named arrays. The array names all begin with a user-provided prefix, so that the user can manage arrays representing different objects in a single object store, zip file, HDF5 file, or Python dictionary. Names are generated by a recursive walk over the type structure, and in fact the type schema is losslessly preserved in the collection of names. This may be useful if an external metadata system is not available.

Data are packed into arrays by recursively applying the following rules to a given name $N$ and type $T$ in the structure, where $N = \mbox{the prefix}$ and $T = \mbox{the whole schema}$ at the root of the type schema.
