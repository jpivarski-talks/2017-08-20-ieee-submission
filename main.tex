...
The goal of this and the HEPQuery project is to make HEP analysis 





easier and faster. ``Easier'' in the sense that data analysts can write high-level Python (or someday declarative) code rather than C++ and ``faster'' in the sense that they not only avoid a performance penalty but can enjoy greater performance with the high-level code. This is the allure of databases: SQL is as abstract as set theory and yet it outperforms most attempts at C++.

For this reason, we have been performance-testing PLUR all through its development, to avoid mistakes, and now provide a 
...
FIXME: one last performance study here, to make it as even-handed as possible with a well-defined methodology.
