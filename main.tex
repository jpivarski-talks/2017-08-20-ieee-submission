...
The goal of this and the HEPQuery project is to make HEP analysis easier to develop and faster to run. We believe that broad type abstractions like PLUR and concise syntax like Python's are easier for data analysts to work with, especially when they are exploring a dataset and focusing on physics and statistics issues, not programming. We also intend to make that interface a performance improvement over idiomatic, object-oriented C++, so that analysts don't have to translate analysis code from a high-level language to a low-level one just to process a large dataset in a reasonable time. This is the allure of databases: SQL is as abstract as set theory and yet it is also faster than most procedural code.

For this reason, we have been performance-testing PLUR all through its development, to avoid mistakes, and now perform an even-handed performance comparison between transformed PLUR code and industry-standard C++ in ROOT. We also break down the differences as much as possible to show where they come from. In doing so, we show that highly tweaked C++ is faster than PLUR, but this is not characteristic of what data analysts would write.

The study measures the time to compute masses of pairs of distinct muons in a collection of HEP collision events (simulated CMS experiment, Drell-Yan process). This is the second example from this paper's motivation, which was too cumbersome for SQL (would require a full outer join). One version of the function is written in Python and transformed by PLUR and Numba to bytecode, the other is written in C++, expecting events and muons to be instances of C++ classes. Both are executed on the same ROOT dataset, extracting arrays for PLUR and using ROOT's built-in deserialization for C++. The dataset is located on a fast SSD disk on an AWS instance, and we use vmtouch\cite{vmtouch} to control whether the data are completely in or completely out of virtual memory.

{\bf FIXME:} actually do the study. I've set up the parameters of the test before doing it, which is good--- almost like a blinded analysis. But I still have to do it.
...
On the surface, this is almost unthinkable--- C++ is supposed to be much faster than an abstract language like Python--- that's why HEP data analysts go to great effort to port their Pythonic explorations into C++. However, the PLUR code transformation and Numba compilation have eliminated Python's disadvantages and given it an advantage not shared by the idiomatic C++, skipping object deserialization. Now procedural code is one step closer to behaving like a database query.

In HEP, we plan to use this tool as part of a database-like query server, delivering analysis results in response to queries written in Python. The PLUR data representation gives us the ability to represent large Lists of HEP events, possibly with deeply nested hierarchies and pointer cross-references, as well as non-event data used to process these events. Possible uses include boosted decision trees and neural nets, expressed as a List of nodes that include Pointers to the List.

The use of columns for data granularity, rather than files that contain columns, opens up possibilities for better efficiency in data storage. It is fairly common for a HEP dataset to have several versions as calibrations of measured attributes improve. Currently, each new version requires as much disk space as the previous, which is wasteful if only a few attributes are improved. With column granularity, a new dataset can be defined using some new and some old columns, a good example of the structural sharing technique of functional languages (or version control systems like git). Additionally, analysts can be more free to try different event selections if each is a List$\langle$Pointer$\langle$Event$\rangle\rangle$, a single column that names events in an uncut, shared dataset.

But perhaps most importantly, the usefulness of this technique is not limited to HEP. The need for complex loop dependencies can hardly be HEP-specific. Although many industries and fields of academia may find their needs mostly met by SQL-like analysis, how much is unexplored simply because the tools are not convenient? It is our wish that this technique finds application in diverse fields, just as the techniques of database-style analysis have inspired our work in HEP.
