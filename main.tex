...
\end{center}
This is more difficult in languages like Python, which do not allow assignment in an expression (the symbol reference can appear anywhere in an expresssion); see our Python implementation (referenced below) for the work-around.
...
{\bf isinstance}: (check type) must return true if the argument has a given type, false otherwise. Since references to non-Primitive PLUR types have all been replaced with integer indexes, a naive application of the {\tt isinstance} function would yield true for integers, contrary to a user's expectation.
...
tuples for unpacking assignments ONLY IMMUTABLE
